...
function profile(walletAddress: string, value: number): WalletBehaviorProfile {
  return { id: `profile:${walletAddress}`, walletAddress, chainIds: ["1"], coverage: { ...coverage, walletAddress }, limitations: [], metrics: [{ name: "concentration", value: { status: "known", value }, confidence: 1, observationCount: 2, recency: "current", evidenceIds: ["balance:1"] }] };
}
...
describe("synthesizeCohort", () => {
  ...
  it("surfaces profiles without eligible metrics as excluded evidence", () => {
    const incomplete = { ...profile("0xc", 0.7), metrics: [{ name: "concentration" as const, value: { status: "unknown" as const, reason: "unavailable" as const }, confidence: 0, observationCount: 0, recency: "current" as const, evidenceIds: [] }] };
