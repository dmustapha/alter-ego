import type { BehaviorMetricName, CohortFinding, CohortSynthesis, WalletBehaviorProfile } from "./types";
...
export function synthesizeCohort(profiles: readonly WalletBehaviorProfile[]): CohortSynthesis {
  ...
  for (const metric of METRICS) {
    ...
    findings.push(Object.freeze({ id: `${kind}:${metric}`, kind, metric, profileIds: Object.freeze(values.map(({ profile }) => profile.id)), value: Object.freeze({ status: "known", value: numbers.reduce((sum, value) => sum + value, 0) / numbers.length }), confidence: values.reduce((sum, { item }) => sum + item.confidence, 0) / values.length, limitations: Object.freeze(kind === "consensus" ? [] : ["Eligible profiles materially disagree."]) }));
  }
