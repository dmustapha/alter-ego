...
Confirmed live via X Layer RPC (`https://rpc.xlayer.tech`, chain 196): `symbol() == "USD₮0"`, `decimals() == 6`. So `price: "$1"` auto-resolves to USDT0, `amount == "1000000"` (1 USDT0 at 6dp; a digits-only integer fee). No address is guessed, it comes from the SDK and matches `docs/LISTING-REJECTION-ANALYSIS.md`.
...
```json
{
  ...
  "accepts": [
    { "scheme": "exact", "network": "eip155:196",
      ...
      "amount": "1000000", "payTo": "<X402_PAYTO_ADDRESS>",
