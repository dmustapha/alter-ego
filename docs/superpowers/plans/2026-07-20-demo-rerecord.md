...
| **Plan 3** — real x402 payment shipped (`/verify` + `/settle`, real USDG on X Layer) | (a) real settlement; (b) `setTimeout` / simulated (Plan 3 is optional) | (a) VO/outro may say "pay per analysis in USDG on X Layer"; (b) VO frames it as "payment-gate integration" and Step 7 script drops any "real payment" claim (already flagged LOW/simulated in `DEMO-SCRIPT.md`). |
